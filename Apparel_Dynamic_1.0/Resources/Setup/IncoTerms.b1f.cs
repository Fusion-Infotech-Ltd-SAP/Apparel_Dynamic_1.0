...
{
    [FormAttribute("Apparel_Dynamic_1._0.Resources.Setup.IncoTerms", "Resources/Setup/IncoTerms.b1f")]
    class IncoTerms : UserFormBase
    {
        public IncoTerms()
        {
        }
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_INCOTRMS"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_INCOTRMS").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_INCOTRMS").GetValue("Name", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Inco Terms Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Inco Terms Name");
